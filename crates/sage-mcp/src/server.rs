...
use sage_core::search::SearchQuery;
use sage_core::search::hybrid::HybridSearcher;
use sage_core::search::ripgrep_fallback;
...
impl McpServer {
    pub fn new(config: SageConfig) -> Self {
        let max_cached = std::env::var("SAGE_MCP_CACHE_SIZE")
            .ok()
    ...
    fn handle_tools_list(&self, id: Option<serde_json::Value>) -> JsonRpcResponse {
        let tools = vec![
            Tool {
                name: "sage_search".into(),
                description: concat!(
                    "Use sage_search INSTEAD of grep/ripgrep for all code search. ",
    ...
    fn handle_tool_call(
        ...
        let result = match tool_name {
            "sage_search" => self.tool_search(&arguments),
            "sage_index" => self.tool_index(&arguments),
            "sage_status" => self.tool_status(&arguments),
            "sage_health" => self.tool_health(&arguments),
    ...
    fn tool_index(&self, args: &serde_json::Value) -> Result<String> {
        ...
        // The caller can use sage_status to check progress.
        self.spawn_background_index(&path);

        Ok(format!(
            "Indexing started for {}. Use sage_status to check progress.",
            path.display()
        ))
    }
    ...
    fn tool_status(&self, args: &serde_json::Value) -> Result<String> {
        ...
        if meta_path.exists() {
            ...
            let meta: sage_core::types::IndexMeta = serde_json::from_str(&content)?;
            Ok(format!(
                "Index exists:\n  Files: {}\n  Chunks: {}\n  Model: {}\n  Updated: {}",
                meta.file_count, meta.chunk_count, meta.embedding_model, meta.updated_at
            ))
        } else {
            Ok("No index found for this project. Run 'sage index' first.".into())
        }
    }
    ...
    fn tool_health(&self, _args: &serde_json::Value) -> Result<String> {
        ...
        if model_manager::is_colbert_downloaded(&models_dir) {
            ...
            status.push("  Run 'sage download-models' to download".to_string());
        }
