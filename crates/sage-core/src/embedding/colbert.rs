...
impl ColbertEmbedder {
    ...
    ///
    /// Thread count is read from `SAGE_ORT_THREADS` (default 4).
    /// CoreML acceleration is gated behind `SAGE_COREML=1`.
    pub fn new(model_dir: &Path) -> Result<Self> {
        let threads: usize = std::env::var("SAGE_ORT_THREADS")
            .ok()
        ...
        #[cfg(target_os = "macos")]
        if std::env::var("SAGE_COREML").is_ok_and(|v| v == "1") {
