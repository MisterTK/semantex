...
mod tests {
    ...
    fn test_roundtrip_search_result_item() {
        let item = SearchResultItem {
            ...
            chunk_type: "AstNode".to_string(),
            name: Some("SageServer".to_string()),
            language: Some("rust".to_string()),
            content: Some("pub struct SageServer {}".to_string()),
        };
        ...
        assert_eq!(deserialized.name.as_deref(), Some("SageServer"));
        assert_eq!(
            deserialized.content.as_deref(),
            Some("pub struct SageServer {}")
        );
    }
