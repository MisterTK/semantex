...
// ─────────────────────────────────────────────────────────────────────────
// v0.6 Item 9 — HyDE retrieval channel (feature-gated, additive)
// ─────────────────────────────────────────────────────────────────────────
//
// Hypothetical Document Embeddings: ask a local LLM to synthesize a short
// code-shaped document that *would* answer the user's NL query, then run
// that synthesized doc through the existing search pipeline as an extra
// retrieval channel. The original-query results and the HyDE results are
// merged here so callers see one ranked list.
//
// This entry point is strictly additive: it does NOT touch the original
// `HybridSearcher::search` pipeline. When the feature is off (default
// build) the function is not compiled in; when it is on but the LLM
// synthesizer errors, we transparently fall back to a plain
// original-query search so the daemon never regresses below the v0.5
// behaviour.

/// v0.6 Item 9 — Run a HyDE-augmented search.
///
/// Synthesizes a hypothetical document via `llm` and folds its top
/// matches into the original query's results. On synthesizer error we
/// silently return the original-query results so the call never fails
/// "harder" than the existing `search` would.
///
/// `max_results` caps the final merged list. The HyDE channel's
/// candidate pool is sized at `max_results.max(8)` and folded in via
/// score-ordered de-dup so existing top hits keep their position when
/// they also appear in the HyDE channel.
#[cfg(feature = "local-llm")]
pub fn hybrid_search_with_hyde(
    searcher: &HybridSearcher,
    query: &str,
    llm: &dyn crate::llm::LlmHyDE,
    max_results: usize,
) -> Result<super::SearchOutput> {
    let max_results = max_results.max(1);

    // Always run the original query — this is the result we return if
    // HyDE synthesis fails.
    let base_query = SearchQuery::new(query).max_results(max_results);
    let base_output = searcher.search(&base_query)?;

    // Try to synthesize a hypothetical doc; fall back to base results on error.
    let hypothetical = match llm.synthesize_doc(query) {
        Ok(doc) if !doc.trim().is_empty() => doc,
        _ => return Ok(base_output),
    };

    // Run the synthesized doc through the same pipeline. We use the
    // doc as the query text — the existing tokenizer + ColBERT encoder
    // handles whatever shape it has.
    let hyde_query = SearchQuery::new(&hypothetical).max_results(max_results.max(8));
    let Ok(hyde_output) = searcher.search(&hyde_query) else {
        // Pipeline failure on the synthesized doc → keep base results.
        return Ok(base_output);
    };

    // Fuse the two channels, keeping base ordering and de-duping by
    // (file, start, end). Pure helper so it's unit-testable.
    let merged = merge_hyde_results(base_output.results, hyde_output.results, max_results);

    // Preserve base_output's metrics shape but reflect the new merged count.
    let mut metrics = base_output.metrics;
    metrics.result_count = merged.len();
    metrics.fused_count = merged.len();
    Ok(super::SearchOutput {
        results: merged,
        metrics,
    })
...
/// Merge the original-query results with HyDE-channel results.
///
/// Base ordering is preserved; HyDE results fill remaining slots up to
/// `max_results`, skipping any chunk that already appears in the base
/// list (matched on (file_path, start_line, end_line)).
///
/// Pulled out as a free function so it's unit-testable without standing
/// up a real `HybridSearcher` — the heavy parts (ColBERT + Tantivy) are
/// not in scope for what this helper actually does.
#[cfg(feature = "local-llm")]
pub fn merge_hyde_results(
    base: Vec<crate::types::SearchResult>,
    hyde: Vec<crate::types::SearchResult>,
    max_results: usize,
) -> Vec<crate::types::SearchResult> {
    // Dedup by chunk.id, not (file, start_line, end_line). Two distinct AST
    // chunks (e.g., an impl block and a method whose body fills it) can share
    // a line range; coord-based dedup would silently drop the second.
    let mut seen: HashSet<u64> = HashSet::new();
    let mut merged: Vec<crate::types::SearchResult> = Vec::with_capacity(max_results);
    for r in base {
    ...
    // Re-sort by score descending so downstream consumers (compute_confidence
    // on items[0]/items[1], triage that assumes ranked input) see a coherent
    // ordering after the two channels were interleaved.
    merged.sort_by(|a, b| {
        b.score
            .partial_cmp(&a.score)
            .unwrap_or(std::cmp::Ordering::Equal)
...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────────
    // v0.6 Item 9 — HyDE merge tests (feature-gated)
    // ─────────────────────────────────────────────────────────────────

    #[cfg(feature = "local-llm")]
    mod hyde_tests {
        use super::super::merge_hyde_results;
        use crate::types::{Chunk, ChunkType, Confidence, SearchResult, SearchSource};
        use std::path::PathBuf;

        /// Derive a stable id from (path, start, end) so two fixtures at the
        /// same span share an id (mimicking how the real index assigns one
        /// chunk.id per AST span) while distinct fixtures get distinct ids —
        /// matches production semantics where chunk.id is the canonical
        /// identity that merge_hyde_results dedupes on.
        fn fixture_id(path: &str, start: u32, end: u32) -> u64 {
        ...
        fn mk_result(path: &str, start: u32, end: u32, score: f32) -> SearchResult {
            SearchResult {
                chunk: Chunk {
                    id: fixture_id(path, start, end),
        ...
        #[test]
        fn merge_dedupes_overlapping_chunk() {
            let base = vec![
                mk_result("a.rs", 1, 10, 1.0),
                mk_result("b.rs", 20, 30, 0.9),
            ];
            let hyde = vec![
                mk_result("b.rs", 20, 30, 0.8),
                mk_result("c.rs", 5, 15, 0.7),
            ];
            let merged = merge_hyde_results(base, hyde, 10);
            assert_eq!(merged.len(), 3);
            // Base ordering preserved for the first two slots.
            assert_eq!(merged[0].chunk.file_path, PathBuf::from("a.rs"));
            assert_eq!(merged[1].chunk.file_path, PathBuf::from("b.rs"));
            // HyDE-only chunk appended after the base list.
            assert_eq!(merged[2].chunk.file_path, PathBuf::from("c.rs"));
        }

        #[test]
        fn merge_caps_at_max_results() {
            let base = vec![mk_result("a.rs", 1, 10, 1.0)];
            let hyde = vec![
                mk_result("b.rs", 1, 10, 0.9),
                mk_result("c.rs", 1, 10, 0.8),
                mk_result("d.rs", 1, 10, 0.7),
            ];
            let merged = merge_hyde_results(base, hyde, 2);
            assert_eq!(merged.len(), 2);
            // base first, then one HyDE result.
            assert_eq!(merged[0].chunk.file_path, PathBuf::from("a.rs"));
            assert_eq!(merged[1].chunk.file_path, PathBuf::from("b.rs"));
        }

        #[test]
        fn merge_with_empty_hyde_returns_base() {
            let base = vec![mk_result("a.rs", 1, 10, 1.0), mk_result("b.rs", 1, 10, 0.5)];
            let merged = merge_hyde_results(base.clone(), vec![], 10);
            assert_eq!(merged.len(), 2);
            assert_eq!(merged[0].chunk.file_path, PathBuf::from("a.rs"));
        }

        #[test]
        fn merge_with_empty_base_uses_hyde() {
            let hyde = vec![mk_result("h.rs", 1, 10, 0.9)];
            let merged = merge_hyde_results(vec![], hyde, 10);
            assert_eq!(merged.len(), 1);
            assert_eq!(merged[0].chunk.file_path, PathBuf::from("h.rs"));
        }
    }
}
