...
use sage_core::server::protocol::{SearchRequest, SearchResultItem};
use sage_core::types::{ChunkType, SearchResult};
...
pub fn run(opts: &SearchOpts, config: &SageConfig) -> Result<()> {
...
fn run_via_daemon(opts: &SearchOpts, project_path: &std::path::Path) -> Result<()> {
    ...
    let response = sage_core::server::daemon_search(project_path, &request)?;
...
fn auto_start_and_search(
    ...
    eprintln!(
        "  (run 'sage serve {}' in the background for instant searches)",
        project_path.display()
    );
    ...
    anyhow::bail!(
        "sage daemon did not become ready within 120 s.\n\
...
fn print_results(
    ...
    eprintln!(
        ...
        results
            .first()
            .map_or(sage_core::types::SearchSource::Dense, |r| r.source),
    );
