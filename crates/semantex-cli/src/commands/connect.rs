...
pub fn run(project_path: &Path) -> Result<()> {
    ...
    let port = semantex_core::server::read_daemon_port(&project_path).context(format!(
        "No daemon running for {}. Start one with 'semantex serve {}'.",
    ...
    eprintln!("Use 'semantex disconnect' to stop.");
