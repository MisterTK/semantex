...
>
> ✅ **DONE 2026-06-01 — hosted + verified.** Set `ModelSpec.source = hf:MisterTK/Qwen3-Reranker-0.6B-onnx` (Apache-2.0). NB it is **fp16, NOT int8** (`shawnw3i`'s source is already quantized — re-quantizing produced an invalid graph; used as-is), so the **download file/sentinel is `model.onnx`, not `model_int8.onnx`** (file list `["model.onnx", "tokenizer.json", "config.json"]`, ~1.1 GB). Recorded facts in `docs/superpowers/plans/2026-05-31-research-notes.md`: `ScoreStrategy::YesNoLogit`, token ids `yes`=**9693** / `no`=**2152**, inputs `input_ids`/`attention_mask`/`position_ids` (int64), output `logits[batch,seq,vocab]`, prompt template verbatim. This spike is now *verify-the-recorded-facts*. A true int8 build (fresh fp32 export) is a deferred optimization.
