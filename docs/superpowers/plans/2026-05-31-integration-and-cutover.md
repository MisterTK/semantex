...
- **D-graph (S4):** add a real **`SEMANTEX_GRAPH_DISABLE`** on/off knob (none exists today) so the "graph-off vs graph-on" gate is measurable. The route names "architectural/exhaustive/feature-planning" are **free-function predicates**, not `QueryType` enum variants (the enum stays 4 variants). S4's SWE-loc gate is measured at **file-level** recall (S0 ships file-level; function-level is an S0 follow-up). **Semantica-review additions (Tasks S4.5/S4.10 — both search-time, off-by-default, S0-measured):** S4.5 wires the already-computed-but-unused `chunk_centrality` PageRank into ranking as a tunable prior (`SEMANTEX_GRAPH_CENTRALITY_WEIGHT`); S4.10 adds import-cohesion ("community") expansion for the exhaustive route (`module_decay`) over the existing `module_edges`. Full **index-time Louvain** community detection is a deferred follow-up (it crosses the index path, out of S4's search-time boundary).
...
- **ONNX artifacts EXIST on HF (verified 2026-05-31) — re-host, don't cold-export.** Permissively-licensed community ONNX exports of both models are already published:
  ...
  ✅ **DONE (2026-06-01) — both hosted under `MisterTK` + CPU-smoke-verified:**
  - Embedder: **`hf:MisterTK/CodeRankEmbed-onnx-int8`** (MIT, **int8**, ONNX external-data → files `model_int8.onnx` + `model_int8.onnx.data` + `tokenizer.json` + `config.json`).
  - Reranker: **`hf:MisterTK/Qwen3-Reranker-0.6B-onnx`** (Apache-2.0, **fp16, NOT int8** — single `model.onnx` + `tokenizer.json` + `config.json`).
  Recorded facts (dims, query prefix, pooling, tensor I/O, yes/no token ids `9693`/`2152`, prompt template, file lists) are in **`2026-05-31-research-notes.md`**. **S2/S3 set `ModelSpec.source` to these repos; the embedder file list MUST include `model_int8.onnx.data`, and the reranker sentinel/file is `model.onnx` (not `model_int8.onnx`).** A true int8 reranker (fresh fp32 export) is a deferred optimization. (Built via `benchmarks/onnx_models/prepare_models.py`.)
...
1. **Dense default (D4):** flip the default `embedder` from **`lateon-colbert`** to **`coderank-137m`** (which routes to the `coderank-hnsw` backend) **only if** it meets-or-beats `colbert-plaid` on CoIR + CSN Recall@10/nDCG@10. If it loses, keep `colbert-plaid` and reassess (do **not** delete ColBERT). **Three-way A/B (semantica review):** the harness also evaluates **`qwen3-embed-0.6b`** (75.41 MTEB-Code, Apache-2.0, ~600 MB int8); pick it over `coderank-137m` only if it wins decisively enough to justify the ~5–8× footprint (a real local-tool-identity cost). Orthogonally, settle the **dense-side-context A/B** (`SEMANTEX_DENSE_CONTEXT`, S2/S0): ship context-on if it lifts nDCG/Recall with no regression — it's a separate index, so it composes with whichever embedder wins.
...
4. **Graph (S4):** ship the tuned decays/hops only with a measured SWE-loc lift and no CoIR/CSN regression. Same gate for the semantica-review additions: ship the **centrality prior** (S4.5, `SEMANTEX_GRAPH_CENTRALITY_WEIGHT`) and **cohesion expansion** (S4.10, `module_decay`) only where each shows a net SWE-loc lift; else leave off-by-default.
