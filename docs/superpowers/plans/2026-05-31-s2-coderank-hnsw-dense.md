...
**Optional A/B extensions (from the 2026-05-31 semantica competitive review — `~/.claude` memory `semantica-review-2026-05-31`). Both default OFF; the S0 harness decides whether either ships.**
- **Dense-side context (contextual-retrieval A/B).** semantex already synthesizes a graph-derived context block per chunk — `chunking/semantic_role.rs::synthesize_nl_annotation` (docstring + `purpose:` + `role:` + `calls:` + `called by:` + impls/inherits) — but feeds it to **BM25 only** (`builder.rs:373-407`); the spec deliberately keeps the dense channel on raw code. The 2026 SOTA (Anthropic Contextual Retrieval, −49% retrieval-failure) and semantica's `embed_subgraph` both indicate dense-side context helps. Add an env-gated switch **`SEMANTEX_DENSE_CONTEXT`** (default off): when on, `encode_document` embeds `format!("{annotation}\n{code}")` instead of raw code (the annotation is passed in from `builder.rs`, already computed for BM25). Keep raw-code default until S0's `--dense-context on|off` ablation proves a net nDCG/Recall lift on CoIR/CSN with no regression. **No new compute** — the annotation already exists. NB: it changes the embedding input → a distinct `embedder_fingerprint` (S8), so context-on/off are separate indexes the harness builds independently.
- **Qwen3-Embedding-0.6B as a third dense candidate.** The SOTA re-check found **Qwen3-Embedding-0.6B (Apache-2.0; ONNX int8 exists) = 75.41 MTEB-Code vs CodeRankEmbed 60.1 CoIR** — the large quality gap spec §7 said would justify revisiting it. It lands as a **built-in `ModelSpec` `qwen3-embed-0.6b`** in S8 (single-vector, dim 1024, mean-pool + L2, `instruction_aware`, int8) routing to the **same `coderank-hnsw` backend** — **no new S2 code**, only an HF source + spec fields — and as a **third arm** of the S0 embedder A/B (`SEMANTEX_EMBEDDER=qwen3-embed-0.6b`). **Honest cost:** ~600 MB int8 (0.6B params) vs CodeRankEmbed ~70–137 MB vs current ColBERT 17 MB — a real "tiny local tool" footprint tradeoff the harness weighs against the quality win; the S8 registry makes the swap a manifest entry.
